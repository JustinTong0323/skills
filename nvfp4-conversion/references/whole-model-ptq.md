...
For W4A4 NVFP4, use the standard recipe by default; Four-Over-Six is an A/B-gated option — see [conversion-contract.md](conversion-contract.md). When converting with 4/6, use `NVFP4_FOUR_OVER_SIX_CFG` / `--qformat nvfp4_four_over_six`, or a custom recipe whose `algorithm` block is `method: mse` with amax multipliers `[1.0, 1.5]` (`step_size: 0.5`, `fp8_scale_sweep: false`) and `four_over_six: true` weight quantizers — the flag alone with `method: max` is plain abs-max, not 4/6. Scope recipes (for example experts-only) compose with 4/6 the same way as with the standard recipe; 4/6 changes only how weight block scales are chosen. The 4/6 weight-side sweep is calibration-data-independent, so a 4/6 conversion needs no calibration forward pass for weight scales.
...
- With dynamic input quantization plus a unit `input_scale` contract and no exported k/v scale tensors, the pipeline degenerates to RTN — a handful of samples (8-64) yields bit-identical expert weights to 1024, and every calibration-produced value (input-side amax) is one you overwrite or strip at assembly anyway. Calibration's real roles here are the forward-path canary (adapter mismatch, NaN, and overflow surface in minutes of calibration forward, not after a 40-minute export) and the manifest's declared calibration identity — not numerics. Use the smallest sample count the unmodified pipeline accepts (8 works); do not run a formal 1,024-sample calibration for this recipe class. A full production gate chain (audit, serve battery, TB2.1 89-task agentic, GSM8K full, AIME26 n=8) passed on a 745B-class GLM MoE converted with 64 samples.
