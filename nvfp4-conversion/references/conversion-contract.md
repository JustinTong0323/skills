...
**The standard recipe is the default; 4/6 is an optional variant gated on a per-family controlled A/B.** The paper's PTQ gains are perplexity and simple-task results on small dense models. Our own production-scale A/B on a 300B-class hybrid MoE reasoning model (unfused experts, 36,423 NVFP4 bases, ModelOpt 0.46.0, official-FP8-scope W4A4) found true 4/6 significantly harmful on hard reasoning tasks: AIME26 88.85% vs 92.45% for the abs-max control (1,920 question×repeat×seed pairs, delta -3.59pt, 95% CI [-5.10, -1.93], McNemar p<0.0001), with +14% median and +22% mean thinking tokens (p90 tail 95,570 vs 54,199) and lower stop rate (92.0% vs 95.2%). GSM8K showed no accuracy difference (-0.13pt [-0.51, +0.25]) but +12% mean thinking. An earlier arm mislabeled as 4/6 (abs-max with 256 normalization) was incidentally the most stable — do not confuse the two. Do not claim 4/6 benefit in a release without a per-family paired A/B under the protocol in [validation-and-release.md](validation-and-release.md) showing non-inferiority on accuracy and thinking-length.
