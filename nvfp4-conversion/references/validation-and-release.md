...
Run `scripts/audit_checkpoint.py` for unified-HF Model Optimizer output. It checks physical/index agreement, recipe metadata coverage, packed and scale forms (dense rank-2 and fused rank-3 expert bases, NVFP4 group size 16), positive finite scales, and logical equality of unchanged tensors in bounded row chunks. Quantized MTP, router, or gate bases fail the audit unless `--allow-quantized-protected` is passed with a dedicated recipe and independent qualification.

The quantized set comes from `quantized_layers` when the metadata carries one (`MIXED_PRECISION` exports, routed `--precision-contract`). For a single-algorithm export it is derived: every source `.weight` base with a `weight_scale` in the output is quantized and must not match an `exclude_modules`/`ignore` glob (matched the way SGLang matches them, on the whole module path or any dotted segment), and every remaining rank-2 or fused rank-3 expert `.weight` base that matches no exclusion and is not a protected MTP/router/gate module fails the audit as neither excluded nor quantized. The derived mode also requires `config.json` `ignore` to equal `hf_quant_config.json` `exclude_modules`, because the runtime reads only the former.
...
Confirm the runtime reads the quantization exclusions you think it reads. When `config.json` carries a `quantization_config`, SGLang resolves exclusions from its `ignore` key alone and never opens `hf_quant_config.json`; only a checkpoint without that block falls back to `exclude_modules` in `hf_quant_config.json`. Official Model Optimizer 0.46 unified exports write both consistently (`nvidia/GLM-5.2-NVFP4` carries the same 156 patterns in each), and `scripts/audit_checkpoint.py` fails when the two sets differ. The trap is a `config.json` `quantization_config` produced without `ignore`: a hand-assembled routed output, a rewritten config, or an older exporter. Exclusions written in checkpoint-namespace prefixes the runtime does not map (VLM `language_model.`, nextn `decoder`, vision `visual.`) fail the same way, and every affected module is built as quantized. A `quantized_layers`-less scope recipe makes this invisible until a packed-vs-BF16 shape assert fires at load. Verify before serving by dumping a few deliberately-unquantized modules' built parameters; an excluded module instantiated as packed U8 with scale tensors means the exclusion metadata was not applied.
...
- sgl-eval sends the same request `seed` to every sample in a run, including all `--n-repeats` samples. Whether same-seed samples come back byte-identical depends on the server's sampling determinism, so probe it first: rerun one small eval against the same server and compare outputs. When repeats are identical they add no power, and multi-seed pairing requires independent CLI-level runs with `--n-repeats 1` and distinct seeds. When the server does not replay same-seed samples identically, repeats act as replicates and pairing by question x repeat index x CLI seed is valid; the paired A/B evidence in [conversion-contract.md](conversion-contract.md) used that pairing. State which regime applied in the qualification record.
