...
This workflow targets Qwen-compatible checkpoints whose routed experts are stored as fused `gate_up_proj` and `down_proj` tensors. Discover all dimensions from source metadata. Do not copy architecture constants, paths, credentials, scheduler names, or private model identities from an earlier run.
...
- SHA256 of source `config.json`, source index, and reference config.
...
- Complete non-routed tensor key set, including MTP keys.
- Worker count and disjoint expert ranges.
- Model Optimizer commit, dependency versions, recipe, and calibration contract.

Abort on a missing key, an unexpected source dtype, an unsupported fused layout, or expert counts that cannot be partitioned safely.
...
At the end of every part, atomically publish the safetensors file followed by an identity-bound completion marker. Release all model, calibration, export, and source-slice references before advancing to the next layer.
...
Rerun the identical command after interruption. Reuse a part only when all identity fields, output size, and output SHA256 match. Quarantine mismatched or incomplete artifacts and regenerate them. Never silently reuse a part across a source, dependency, recipe, calibration, or partitioning change.

### 5. Assemble On CPU

Verify every part and marker before copying source data. Replace only the original fused routed expert tensors. Copy all remaining source tensors unchanged, preserve MTP in BF16, merge the reference exclusion list, remove KV-cache quantization declarations, and generate complete configs and index files.
...
- Source and Model Optimizer identities.
